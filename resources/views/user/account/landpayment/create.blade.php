@extends('layouts.app')
    ...
    <div class="single-pro-review-area mt-t-30 mg-b-15">
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <h3 class="text-center">Land Payment Details</h3>
                    <form action="{{ route('account.land_payment.store') }}" class="acount-infor" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('POST')
                    <div class="row">
                        <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                            <div class="form-group">
                                <label>Land ID </label>
                                <select name="landcustomers_id" id="landcustomers_id" class="form-control">
                                        <option value="">Select Land ID</option>
                                        @foreach($customers as $customer)
                                        <option value="{{$customer->id}}">SKSLD {{$customer->id}}</option>
                                        @endforeach
                                </select>
                                @error('landcustomers_id')
